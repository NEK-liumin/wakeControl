...
// nu.y = tau * Y^-1 * e - mu
// nu.mu = bI - AIx + y
int getNuBeforePre(Nu& nu, ICQP problem)
{
	Column Hx;
	Column AITmu;
	getAB(Hx, problem.H, problem.x);
...
int getStep(double& alphaP, double& alphaD, Delta& delta, ICQP& problem)
{
	alphaP = LARGE;
	alphaD = LARGE;
	double eta = 1 - 1e-3;
...
int getTau(double& tau, ICQP& problem, Delta delta_pre, double& alphaP_pre, double& alphaD_pre)
{
	double tauk;
	double tau_pre;
	getATB(tauk, problem.mu, problem.y);
...
bool getDelta_cor(Delta& delta_cor, EICQP& problem, ECQP& QP_cor, Nu& nu, Matrix& Yinverse, Matrix& M)
{
	bool isSolvable;
...
int getDelta(Delta& delta, Delta& delta_pre, Delta& delta_cor, EICQP& problem)
{
...
int getNewPoint(ICQP& problem, Delta& delta, double& alphaP, double& alphaD)
{
	getAlphaA(delta.x, alphaP);
	getAlphaA(delta.y, alphaP);
	getAlphaA(delta.mu, alphaD);
...
int getNewInterval(double& interval, ICQP& problem)
{
	getATB(interval, problem.mu, problem.y);
	return 0;
}
